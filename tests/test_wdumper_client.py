...
def test_get_dump_calls_correct_url(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
...
def test_get_dump_passes_accept_json_header(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    client, mock_session = make_client()
    client.get_dump(DUMP_ID)
...
def test_get_dump_forwards_cache_duration(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    client, mock_session = make_client()
...
def test_get_dump_returns_url_and_dump(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
...
def test_get_dump_parses_string_spec(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    spec = {"labels": True}
    json_data = {
        "dump": {"id": DUMP_ID, "title": "My Dump", "spec": json.dumps(spec)}
    }
    client, _ = make_client(json_data=json_data)
    _, dump = client.get_dump(DUMP_ID)
    assert dump["spec"] == spec  # type: ignore[comparison-overlap]


def test_get_dump_leaves_dict_spec_unchanged(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    spec = {"labels": True}
...
def test_get_dump_raises_client_error_on_request_exception(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    client, mock_session = make_client()
...
def test_get_dump_raises_client_error_on_bad_status(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
...
def test_get_dump_raises_client_error_on_invalid_json_response(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    client, mock_session = make_client()
    mock_session.get.return_value.json.side_effect = (
        json.decoder.JSONDecodeError("err", "", 0)
    )
    with pytest.raises(ClientError):
        client.get_dump(DUMP_ID)


def test_get_dump_raises_client_error_on_invalid_spec_json(
    make_client: Callable[..., tuple[WDumperClient, MagicMock]],
) -> None:
    json_data = {
        "dump": {"id": DUMP_ID, "title": "My Dump", "spec": "not valid json"}
    }
    client, _ = make_client(json_data=json_data)
    with pytest.raises(ClientError):
        client.get_dump(DUMP_ID)
